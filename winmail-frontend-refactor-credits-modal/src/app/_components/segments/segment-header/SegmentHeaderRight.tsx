...
const SegmentHeaderRight = () => {
  ...
  const handleOpenAddContactModal = async () => {
    // Fetch latest user to avoid stale credits after purchase
    let latestCredits: number | undefined = user?.credits;
    try {
      const res = await getUser();
      if (res?.data && typeof res.data.credits === 'number') {
        latestCredits = res.data.credits;
      }
    } catch {}

    // Frontend guard: block if credits < 0.02
    if (typeof latestCredits === 'number' && latestCredits < 0.02) {
      toast({
